...
M_q_dependence=[120,0];%(GI_LSTM) Memory-group max dependencies [q1,q2]

network_size_type=1;%predefined network size type (2^(network_size_type-1)).
sub_seq_length=400;%length of the subsequences to be used.

Flag_mM_sd=1;%Normalization Flag (0: minmax, 1:mean_std)
Flag_u_g=0;%Probability distribution for NN weights (0:uniform, 1:gaussian)

plotting_flag=0;%Visualization of the validation set fitting process.
...
Flag_val_enable=1;
% Number_val_fails is the number of semi-consecutive increasing
% errors allowed before stopping the training.
...
%####################GI_LSTM architecture construction####################%
%Array containing the number of units for different predefined network sizes.
number_units_array=2.^[0:8];%Experimental predefined GI_LSTM network sizes (powers o 2)
number_hidden_units=number_units_array(network_size_type);%number of units in the GI_LSTM
number_hidden_layers=size(number_hidden_units,2);%Number of hidden layers in the GI_LSTM
...
%##########stroring variables for the Training process's results##########%
E_Meta_train=zeros(number_repetitions,1);
E_Meta_val=zeros(number_repetitions,1);
E_Meta_testing=zeros(number_repetitions,1);
Iterations_optimal=zeros(number_repetitions,1);
%#########################################################################%
...
%##########################Experiment-repetition loop level##########################%
for i_repetition=1:number_repetitions
    disp(['repetition:',num2str(i_repetition)]);
    ...
    %%%%%%%%%%%%%%%%%%%%%%GI_LSTM training-process auxiliary variables%%%%%%%%%%%%%%%%%%%%%
    %///////////////////Adam optimizer's hyperparameters////////////////////%
    betha1=0.9;
    betha2=0.999;
    betha1_acum=1;
    betha2_acum=1;
    ...
    %/////////////////////Cell_state_saturation_threshold///////////////////////%
    Scaling_DelC_th=10000;
    ...
    %//////////////////Forward-pass temporary-information storing variables////////////////%
    ...
    Cluster_F1=cell(number_hidden_layers,2);%forget gate1 values
    Cluster_F2=cell(number_hidden_layers,2);%forget gate2 values
    Cluster_F1_temporal=cell(number_hidden_layers,size(Xn,1));%forget gate1 past values
    Cluster_F2_temporal=cell(number_hidden_layers,size(Xn,1));%forget gate2 past values
    Array_w_epoch=zeros(number_M_parameters(1),number_epochs,number_hidden_layers);%GI_LSTM interpretable-weights1 variables 
    ...
    %//////////////////Backward-pass temporary-information storing variables///////////////%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%GI_LSTM Training loop%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    tic
    for i_epoch=1:number_epochs

        alpha = ones(number_hidden_layers,1)*Learning_rate;
        cont_sub_seq=sub_seq_length;
        Flag_stop=1;
        i_sub_seq=0;
        Scaling_DelC_prev_aux=Scaling_DelC_th*ones(number_hidden_layers,ceil(size(Xn,1)/sub_seq_length));
        while(Flag_stop)
            ...
            %////////////////////////////////////////////Forward pass////////////////////////////////////////////%
            sub_seq_Xn=Xn(cont_sub_seq-sub_seq_length+1:cont_sub_seq,:)';
            sub_seq_Yn=Yn(cont_sub_seq-sub_seq_length+1:cont_sub_seq,:)';
            for i_b_s=1:sub_seq_length
                for i_h_l=1:number_hidden_layers

                    if (cont_sub_seq-sub_seq_length>0)&&(i_b_s==1)
                    ...
                    if cont_sub_seq-sub_seq_length+i_b_s==1
                        if i_h_l==1
                    ...
                    %---------------Gates' Non-linear transformations-----------------%
                    ...
                    %-----------------------------------------------------------------%
                    Cluster_F1_temporal{i_h_l,cont_sub_seq-sub_seq_length+i_b_s}=...
                        Cluster_F1{i_h_l,1};
                    Cluster_F2_temporal{i_h_l,cont_sub_seq-sub_seq_length+i_b_s}=...
                        Cluster_F2{i_h_l,1};
                    %-----------------------LSTM input update-------------------------%
                    ...
                    if cont_sub_seq-sub_seq_length+i_b_s>1
                        if (~isempty(number_M_parameters))
                            ...
                            elseif(length(number_M_parameters)>1)&&(cont_sub_seq-sub_seq_length-number_M_parameters(1)>1)%This assumes that sub_seq size is bigger than number_M_parameters(1)
                                ...
                                Matrix_layers_C(row_CM(i_h_l,1):row_CM(i_h_l,2),i_b_s)=...
                                    ...
                                    a_f.*Cluster_F1{i_h_l,1}.*Matrix_layers_M1(row_CM(i_h_l,1):row_CM(i_h_l,2),i_b_s)+...
                                    b_f.*Cluster_F2{i_h_l,1}.*Matrix_layers_M2(row_CM(i_h_l,1):row_CM(i_h_l,2),i_b_s);
                            elseif(cont_sub_seq-sub_seq_length>1)%This assumes that sub_seq size is bigger than number_M_parameters(1)
                                aux_i_aug_C=i_b_s-(1:number_M_parameters(1));
                                Matrix_layers_C_increased=[Matrix_layers_C(row_CM(i_h_l,1):row_CM(i_h_l,2),aux_i_aug_C(aux_i_aug_C>0)),...
                                    Matrix_layers_C_prev_sub_seq(row_CM(i_h_l,1):row_CM(i_h_l,2),end+aux_i_aug_C(aux_i_aug_C<=0))];
                                Matrix_layers_M1(row_CM(i_h_l,1):row_CM(i_h_l,2),i_b_s)=...
            ...
            %-----------Error matrix linked to the loss function MSE--------------%
            ...
            for i_h_l=1:number_hidden_layers
                ...
                for i_b_s=1:sub_seq_length
                    ...
                    %----------------selection of the input sequence values to the LSTM units-----------------%
                    ...
                    %---------------------------Cell-state gradient saturation--------------------------------%
                ...
                Scaling_DelC_prev(end+1-i_h_l,i_sub_seq+1)=...
                    Scaling_DelC_prev_aux(end+1-i_h_l,i_sub_seq+1);
                if (mod(i_epoch,10)==0)&&(i_sub_seq+1==ceil(size(Xn,1)/sub_seq_length))
                ...
                for i_aux1=1:4+2*length(number_M_parameters)
                    ...
                    %-----------------------------------------------------------------------------------------%
                    Cluster_V{end+1-i_h_l,i_aux1}=Cluster_V{end+1-i_h_l,i_aux1}+alpha(end+1-i_h_l)*aux_Momentum_V;
                    %-----------------------------------------------------------------------------------------%
                end
                %---------------------------------------------------------------------------------------------%
            end
            ...
            %--------------------------GI_LSTM Memory-groups' weights normalization--------------------------%
            for i_h_l=1:number_hidden_layers
                if (~isempty(number_M_parameters))
            ...
            Cluster_Vy{1,1}=Cluster_Vy{1,1}+alpha(end+1-i_h_l)*aux_momentum_Vy;
            %-----------------------------------------------------------------------------------------%
        ...
        if Flag_val_enable==1
            for i_h_ia=1:number_hidden_layers
                Array_w_epoch(:,i_epoch,i_h_ia)=...
                    mean(f_mean_max(:,1,i_h_ia).*abs(Cluster_V{1,6})./sum(abs(Cluster_V{1,6}),2),1);
                if length(number_M_parameters)>1
                    Array_w_epoch2(:,i_epoch,i_h_ia)=[mean(f_mean_max(:,1,i_h_ia)),...
                        mean(f_mean_max(:,2,i_h_ia).*abs(Cluster_V{1,8})./sum(abs(Cluster_V{1,8}),2),1)];
                end
            end
            [~,Matrix_error_train,C_train_final,M1_train_final,H_train_final]=GI_LSTM_evaluation(X_train,Y_train,RX,RY,Cluster_V,Cluster_Vy,number_hidden_units,number_hidden_layers,0,[],[],[],number_M_parameters);
            [Ye_val,Matrix_error_val,C_val_final,M1_val_final,H_val_final]=GI_LSTM_evaluation(X_val,Y_val,RX,RY,Cluster_V,Cluster_Vy,number_hidden_units,number_hidden_layers,1,C_train_final,M1_train_final,H_train_final,number_M_parameters);
            [~,Matrix_error_testing,~,~,~]=                                GI_LSTM_evaluation(X_testing,Y_testing,RX,RY,Cluster_V,Cluster_Vy,number_hidden_units,number_hidden_layers,1,C_val_final,M1_val_final,H_val_final,number_M_parameters);
            ...
            if mod(i_epoch,plotting_freq_epoch)==0

                if plot_heat_on==1
                    figure(4)
                    h1=heatmap(Array_w_epoch(:,1:i_epoch)./...
                        max(Array_w_epoch(:,1:i_epoch),[],1),'CellLabelColor','none');
                    h1.GridVisible=0;
                    XLabels = 1:i_epoch;
                    ...
                    if length(number_M_parameters)>1
                        figure(5)
                        h2=heatmap(Array_w_epoch2(:,1:i_epoch)./...
                            max(Array_w_epoch2(:,1:i_epoch),[],1),'CellLabelColor','none');
                        h2.GridVisible=0;
    ...
    E_Meta_testing(i_repetition,1)=norm(Ye_testing-Y_testing)/sqrt(length(Y_testing));
    toc
    disp(['Experiment num: ',num2str(i_repetition),' finished']);
    disp(['Training error:',num2str(E_Meta_train(i_repetition,1))]);
    disp(['Validation error:',num2str(E_Meta_val(i_repetition,1))]);
    disp(['Testing error:',num2str(E_Meta_testing(i_repetition,1))]);
